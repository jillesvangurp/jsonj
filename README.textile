...
The JsonElement interface specifies a lot of additional methods that allow you to do easy type checks and to convert to Java native type (when needed), etc. The API has been finetuned over many months of using it for real work and I've aimed to elemenate the need for any code that feels like it is overly repetitive or verbose. I regard this as the *key selling point of the library*.
...
* It implements Serializable as well, if you really want to use that (hint, you shouldn't).
...
* 1.11
** Add simple JsonSet implementation because sometimes it is just nice to have one. This is a simple variation of the JsonArray that is not supported during parsing.
